...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x, gamma, beta, eps, x_normalized, sample_mean, sample_var)
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    x, gamma, beta, eps, x_normalized, mean, var = cache
    N, D = dout.shape
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (gamma / (N * np.sqrt(var + eps))) * (N * dout - dbeta - x_normalized * dgamma)
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    dx_norm = dout * gamma
    sum_dx_norm = np.sum(dx_norm, axis = 1, keepdims = True)
    sum_dx_norm_x_norm = np.sum(x_norm * dx_norm, axis = 1, keepdims = True)
    dx = (1.0 / (D * np.sqrt(var + eps))) * (D * dx_norm - sum_dx_norm - x_norm * sum_dx_norm_x_norm)
...
def dropout_forward(x, dropout_param):
    ...
    if mode == "train":
        ...
        #######################################################################
        # mask = np.random.binomial(n=1, p=p, size=x.shape) <- có thể tạo trực#
        # tiếp như thế này nhưng mà hình như cách này sẽ làm tăng thời gian tính toán
